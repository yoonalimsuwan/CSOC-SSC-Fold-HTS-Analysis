# =============================================================================
# CSOC‑SSC v24.1 — Production SOC‑Driven Neural‑Physical Folding Engine
# =============================================================================
# Author: Yoon A Limsuwan
# License: MIT
# Year: 2026
#
# V24.1 patches all critical issues found in V24: adds sinusoidal positional
# encoding, centres training targets, couples the SOC kernel into the energy,
# computes the neural restraint once, and simplifies the avalanche gradient
# flow.  The engine remains fully distributed (DDP) with FlashAttention,
# correct physics, and SOC dynamics.
#
# USAGE
# -----------------------------------------------------------------------------
#   torchrun --nproc_per_node=4 csoc_v24_1.py train
#   python csoc_v24_1.py refine --pdb 1UBQ
#   python csoc_v24_1.py refine --seq ACDEFGHIK...
# =============================================================================
...
# Logging
# ──────────────────────────────────────────────────────────────────────────────
def setup_logger(name: str = "CSOC‑SSC_V24.1", local_rank: int = -1) -> logging.Logger:
...
class V24Config:
    ...
    w_soc_contact: float = 0.3         # small coupling of SOC kernel to energy
...
class PDBFetcher:
    ...
    def fetch_and_parse(pdb_id: str) -> Backbone:
        ...
        req = urllib.request.Request(url, headers={'User-Agent': 'CSOC-SSC_V24.1_Research'})
...
# ──────────────────────────────────────────────────────────────────────────────
# Positional Encoding (sinusoidal) – added in V24.1
# ──────────────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────────────
# Neural Modules (FlashAttention + Positional Encoding)
# ──────────────────────────────────────────────────────────────────────────────
...
class FlashSequenceEncoder(nn.Module):
    def __init__(self, dim: int, depth: int, heads: int, ff_mult: int):
        ...
        self.pos_enc = SinusoidalPositionalEncoding(dim)   # V24.1 addition
        self.layers = nn.ModuleList([
            FlashGeometryBlock(dim, heads, ff_mult) for _ in range(depth)
        ])

    def forward(self, seq_ids: torch.Tensor) -> torch.Tensor:
        x = self.embed(seq_ids)                     # (B, L, D)
...
# ──────────────────────────────────────────────────────────────────────────────
# SOC Kernel & CSOC Controller (unchanged from V23/V24)
# ──────────────────────────────────────────────────────────────────────────────
class SOCKernel:
...
# ──────────────────────────────────────────────────────────────────────────────
# Physics Energy Terms (V23 Corrected, V24.1 SOC coupling added)
# ──────────────────────────────────────────────────────────────────────────────
...
def contact_energy_from_kernel(ca, kernel, cfg):
    """SOC kernel energy: K large → should be close → -K * exp(-D/λ) penalises when far."""
    D = torch.cdist(ca, ca)
...
def total_physics_energy(ca, seq, alpha, kernel, cfg):
    ...
    if cfg.use_soc_kernel and kernel is not None:
        e += contact_energy_from_kernel(ca, kernel, cfg)
    return e

# ──────────────────────────────────────────────────────────────────────────────
# Datasets (now centre targets)
# ──────────────────────────────────────────────────────────────────────────────
...
class CSOCSSC_V24_1(nn.Module):
    ...
    def _soc_avalanche(self, coords: torch.Tensor, alpha: torch.Tensor, kernel: torch.Tensor):
        # Assumes coords.grad is populated (after loss.backward())
    ...
    def refine(self, sequence: str, init_coords=None, steps=None, logger=None):
        ...
        # 1. Initial coordinates & alpha
        if init_coords is not None:
            # centre initial coords (V24.1)
        ...
        # 2. Neural restraint (computed once, V24.1)
        neural_target = None
        if init_coords is None:   # only if starting from prediction
            with torch.no_grad():
        ...
        for step in range(steps):
            ...
            with autocast(device_type=device.type, enabled=self.cfg.use_amp):
                ...
                if neural_target is not None:
                    e_reg = 0.1 * ((coords - neural_target) ** 2).mean()
                    loss = loss + e_reg
                else:
                    e_reg = torch.tensor(0.0, device=device)

            scaler.scale(loss).backward()          # no retain_graph (V24.1)
            scaler.unscale_(opt)
            torch.nn.utils.clip_grad_norm_([coords], max_norm=10.0)
            scaler.step(opt)
            scaler.update()

            # SOC avalanche (uses coords.grad, which is still valid)
            if step % 20 == 0 and step > 0 and K is not None:
                self._soc_avalanche(coords, alpha, K)
...
def train_model(model: nn.Module, dataloader: DataLoader, cfg: V24Config, logger: logging.Logger):
    ...
    for epoch in range(cfg.epochs):
        ...
        for step, (seq_ids, target_coords) in enumerate(dataloader):
            ...
            with autocast(device_type=device.type, enabled=cfg.use_amp):
                ...
                # both pred_coords and target_coords are already centred
    ...
    if cfg.local_rank in [-1, 0]:
        ...
        path = os.path.join(cfg.checkpoint_dir, "v24_1_pretrained.pt")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="CSOC‑SSC v24.1 Distributed Folding Engine")
    ...
    refine_parser.add_argument('--out', type=str, default='refined_v24_1.pdb')
    refine_parser.add_argument('--steps', type=int, default=600)
    refine_parser.add_argument('--checkpoint', type=str, default='v24_1_pretrained.pt')
    ...
    logger = setup_logger("CSOC-SSC_V24.1", local_rank)

    if local_rank in [-1, 0]:
        logger.info("=" * 60)
        logger.info(f"CSOC-SSC V24.1 Distributed Folding Engine initialised")
    ...
    if args.command == 'train':
        ...
        model = CSOCSSC_V24_1(cfg).to(torch.device(device_str))
        if is_distributed:
        ...
        if args.pdb:
            ...
            if coords_list:
                ...
                logger.info(f"Loaded {len(init_coords)} CA atoms from local file.")
        if not target_seq:
            raise ValueError("Must provide --seq or --pdb for refinement.")
