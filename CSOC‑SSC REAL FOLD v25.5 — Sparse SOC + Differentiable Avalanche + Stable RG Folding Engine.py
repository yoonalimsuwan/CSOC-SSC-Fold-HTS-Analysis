# =============================================================================
# CSOC‑SSC v25.5 — Sparse SOC + Differentiable Avalanche + Stable RG Folding Engine
# =============================================================================
# Author: Yoon A Limsuwan
# License: MIT
# Year: 2026
#
# V25.5 replaces the dense O(N²) SOC kernel with a sparse neighbour list,
# converts the SOC avalanche into a fully differentiable loss term, and
# stabilises the RG refinement.  All other V24.1 features (positional encoding,
# centred targets, FlashAttention, DDP, PDB fetching) are retained.
# =============================================================================
...
# Logging
# ──────────────────────────────────────────────────────────────────────────────
def setup_logger(name="CSOC‑SSC_V25.5", local_rank=-1):
...
class V25Config:
    ...
    w_avalanche: float = 0.2          # weight for differentiable avalanche loss
    ...
    # Sparse neighbour list parameters
    sparse_cutoff: float = 12.0      # cutoff radius for sparse SOC graph (Å)
    ...
    checkpoint_dir: str = "./v25_5_ckpt"
    out_pdb: str = "refined.pdb"

# ──────────────────────────────────────────────────────────────────────────────
# Data Structures & PDB Fetching (unchanged from V24.1)
# ──────────────────────────────────────────────────────────────────────────────
@dataclass
class Backbone:
    ca: np.ndarray
...
class PDBFetcher:
    ...
    def fetch_and_parse(pdb_id: str) -> Backbone:
        ...
        req = urllib.request.Request(url, headers={'User-Agent': 'CSOC-SSC_V25.5_Research'})
...
# ──────────────────────────────────────────────────────────────────────────────
# Sparse Neighbour List for SOC kernel (O(N log N) using kd‑tree)
# ──────────────────────────────────────────────────────────────────────────────
class SparseNeighborList:
    def __init__(self, cutoff=12.0):
        self.cutoff = cutoff

    def build(self, coords_np: np.ndarray) -> Tuple[torch.Tensor, torch.Tensor, List[List[int]]]:
        """coords_np: (L,3) numpy array.
        Returns:
            edge_index: (2, E) tensor of (src, dst) pairs.
            edge_dist: (E,) tensor of distances.
            adj: list of lists, adj[i] contains indices of neighbours of i.
        """
        L = coords_np.shape[0]
        tree = cKDTree(coords_np)
        # query all points within cutoff
        rows, cols, dists = [], [], []
        adj = [[] for _ in range(L)]
        for i in range(L):
            # query ball point returns indices of neighbours within radius
            nbrs = tree.query_ball_point(coords_np[i], self.cutoff)
            for j in nbrs:
                if j == i:
                    continue
                rows.append(i)
        ...
        return edge_index, edge_dist, adj
...
# ──────────────────────────────────────────────────────────────────────────────
# Differentiable RG Refinement (safe version)
# ──────────────────────────────────────────────────────────────────────────────
class DiffRGRefiner:
    ...
    def forward(self, coords: torch.Tensor) -> torch.Tensor:
        ...
        L = coords.shape[0]
        f = self.factor
        m = L // f * f  # trim to multiple
        if m == 0:
            return coords
        coords_trim = coords[:m].permute(1, 0).unsqueeze(0)  # (1,3,m)
        # average pooling
        pooled = F.avg_pool1d(coords_trim, kernel_size=f, stride=f)  # (1,3,m//f)
        # interpolate back to L
        up = F.interpolate(pooled, size=L, mode='linear', align_corners=True)  # (1,3,L)
        return up.squeeze(0).permute(1, 0)

# ──────────────────────────────────────────────────────────────────────────────
# Backbone Geometry Tools (unchanged)
# ──────────────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────────────
# Physics Energy Terms (with sparse SOC contact)
# ──────────────────────────────────────────────────────────────────────────────
...
def sparse_soc_energy(ca, alpha, edge_index, edge_dist, cfg):
    ...
    aj = alpha[dst]
    a = 0.5 * (ai + aj)
    # kernel value: r^{-a} * exp(-r/lambda)
    K = torch.exp(-a * torch.log(edge_dist + 1e-8)) * torch.exp(-edge_dist / cfg.kernel_lambda)
    # energy: -K * exp(-r/8)
...
# ──────────────────────────────────────────────────────────────────────────────
# Differentiable Avalanche Loss
# ──────────────────────────────────────────────────────────────────────────────
def avalanche_loss(coords, alpha, adj, edge_index, edge_dist, cfg):
    """
    For each residue whose gradient stress exceeds threshold, add a loss that
    pushes its top‑k neighbours (selected by SOC kernel weight) along the
    negative gradient direction.  This is fully differentiable.
    """
    if coords.grad is None:
        return torch.tensor(0.0, device=coords.device)
    L = coords.shape[0]
    stress = torch.norm(coords.grad, dim=-1)   # per‑residue gradient norm
    high = stress > cfg.avalanche_threshold
    if not high.any():
        return torch.tensor(0.0, device=coords.device)

    # compute kernel values for all edges
    src, dst = edge_index[0], edge_index[1]
    ai = alpha[src]
    aj = alpha[dst]
    a = 0.5 * (ai + aj)
    K_edge = torch.exp(-a * torch.log(edge_dist + 1e-8)) * torch.exp(-edge_dist / cfg.kernel_lambda)

    loss = torch.tensor(0.0, device=coords.device)
    stressed_idx = torch.where(high)[0]
    for i in stressed_idx:
        # get neighbours of i from adjacency list (adj is Python list of lists)
        nbrs = adj[i]
        if not nbrs:
            continue
        # map local neighbour indices to edge index
        # we need to find which edges have src==i and dst in nbrs.
        # To avoid slow search, we can precompute a per‑node edge list.
        # For simplicity, we'll build a mask over edges: (src == i) & (dst in nbrs)
        # Since nbrs are from adj, we can convert to set and filter.
        # This may be slightly slow; but avalanche steps are only every 20 iterations.
        nbr_set = set(nbrs)
        edge_mask = (src == i) & torch.tensor([(j in nbr_set) for j in dst.tolist()], device=coords.device)
        if not edge_mask.any():
            continue
        # get kernel values for those edges
        k_vals = K_edge[edge_mask]
        # select top‑k
        k_val, idx = torch.topk(k_vals, min(cfg.avalanche_topk, len(k_vals)))
        # corresponding neighbour indices
        nbr_indices = dst[edge_mask][idx]
        # direction = negative gradient at i (normalized)
        direction = -coords.grad[i]
        dir_norm = torch.norm(direction)
        if dir_norm < 1e-6:
            continue
        direction = direction / dir_norm
        # weight = kernel value / sum(kernel values) for selected neighbours
        weight = k_val / (k_val.sum() + 1e-8)
        # loss: - weight * (coords[nbr] · direction)   (encourages moving along direction)
        loss = loss - (weight * (coords[nbr_indices] * direction).sum(dim=-1)).sum()
    return cfg.w_avalanche * loss

# ──────────────────────────────────────────────────────────────────────────────
# Total Physics Energy Aggregator
# ──────────────────────────────────────────────────────────────────────────────
def total_physics_energy(ca, seq, alpha, edge_index, edge_dist, adj, cfg):
    ...
    if edge_index is not None and edge_index.numel() > 0:
        ...
        e += avalanche_loss(ca, alpha, adj, edge_index, edge_dist, cfg)
    return e

# ──────────────────────────────────────────────────────────────────────────────
# Datasets (centred targets)
# ──────────────────────────────────────────────────────────────────────────────
...
# V25.5 Core Model
# ──────────────────────────────────────────────────────────────────────────────
class CSOCSSC_V25_5(nn.Module):
    def __init__(self, cfg: V25Config):
        super().__init__()
        self.cfg = cfg
        ...
        self.neighbor_list = SparseNeighborList(cutoff=cfg.sparse_cutoff)
    ...
    def refine(self, sequence, init_coords=None, steps=None, logger=None):
        ...
        # Build sparse neighbour list once (coordinates will change, but structure is large-scale;
        # we can rebuild periodically, e.g., every 50 steps)
        edge_index, edge_dist, adj = self._build_sparse_graph(coords.detach().cpu().numpy())
        ...
        for step in range(steps):
            ...
            with autocast(device_type=device.type, enabled=self.cfg.use_amp):
                e_phys = total_physics_energy(coords, sequence, alpha,
                                             edge_index.to(device), edge_dist.to(device), adj, self.cfg)
                loss = e_phys
            ...
            # Periodic rebuild of sparse graph (every 50 steps)
    ...
    def _build_sparse_graph(self, coords_np):
        edge_index, edge_dist, adj = self.neighbor_list.build(coords_np)
        return edge_index, edge_dist, adj
...
def train_model(model, dataloader, cfg, logger):
    ...
    if cfg.local_rank in [-1, 0]:
        ...
        path = os.path.join(cfg.checkpoint_dir, "v25_5_pretrained.pt")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="CSOC‑SSC V25.5 Distributed Folding Engine")
    ...
    refine_parser.add_argument('--out', type=str, default='refined_v25_5.pdb')
    refine_parser.add_argument('--steps', type=int, default=600)
    refine_parser.add_argument('--checkpoint', type=str, default='v25_5_pretrained.pt')
    ...
    cfg = V25Config(local_rank=local_rank, is_distributed=is_distributed, device=device_str,
    ...
    logger = setup_logger("CSOC-SSC_V25.5", local_rank)

    if local_rank in [-1,0]:
        logger.info("="*60)
        logger.info(f"CSOC-SSC V25.5 – Sparse SOC + Differentiable Avalanche")
    ...
    if args.command == 'train':
        ...
        model = CSOCSSC_V25_5(cfg).to(torch.device(device_str))
        if is_distributed:
